...
import Data.Kripke.Kripke (Atom(..), Evaluation(..), Model, Node(..), testAccess, testFact)
...
evaluate { valuation } w (Var v) = testFact valuation (Atom v) w
...
evaluate m@{ frame: { worlds, relation } } w (Implies x1 x2) = and $ map (evaluate' x2) accessibleSatisfying
  where evaluate' expr world = evaluate m world expr
        accessibleSatisfying = filter ((&&) <$> testAccess relation w <*> evaluate' x1) worlds
